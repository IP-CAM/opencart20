...
abstract class ControllerPaymentSmart2payAbstract extends Controller
{
    ...
    public function index( $data = false )
    {
        ...
        {
            // we are in public part...
            return $this->index_public();
        }
    ...
    protected function index_public()
    {
        if( defined( 'DIR_CATALOG' ) )
            return '';
        ...
        $data['base_img_url'] = '';
        if( ($images_dir = $this->model_smart2pay_helper->get_template_dir_location( 'image/payment/smart2pay/methods' ))
        and is_array( $images_dir ) )
        {
            if( !empty( $images_dir['url'] ) )
                $data['base_img_url'] = $images_dir['url'];
            else
                $data['base_img_url'] = $images_dir['default_url'];
        }
        ...
        $data['pay_link'] = $this->url->link( 'payment/smart2pay/pay', (!empty( $this->session->data['token'] )?'token=' . $this->session->data['token'].'&':'').'method='.$this->get_method_id(), 'SSL' );

        return $this->load->view( $template_file, $data );
    }

    /**
     * Empty method that helps us instantiate a payment module without running anything
     *
     * @return ControllerPaymentSmart2payAbstract $this
     */
    public function get_my_instance()
    {
        return $this;
    }
    ...
    public function install()
    {
        ...
        $this->session->data['success'] = 'Successfully installed Smart2Pay '.$this->get_method_name().'!';
        $this->response->redirect( $this->url->link( 'payment/smart2pay/view_payment_methods', 'token=' . $this->session->data['token'].'&installed='.$this->get_method_short_name(), 'SSL' ) );
    }
